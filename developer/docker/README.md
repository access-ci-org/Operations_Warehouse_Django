...
docker build -f ./warehouse.yml -t warehouse:latest --progress=plain --no-cache . 2>&1 | tee build_warehouse.log
# Start the containers
docker-compose -f warehouse_deploy.yml up -d
```

---
> NOTE: For all of the below commands, if you are using docker-compose v1, you
will need to use the -dc1.sh scripts in ./local_scripts, as docker-compose v1
uses underscores instead of hyphens in the container names.
To check your version, run: `docker-compose version` OR `docker compose
version`
...
Open a bash shell on the django warehouse container
```
